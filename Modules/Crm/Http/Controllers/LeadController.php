...
class LeadController extends Controller
{
    ...
    $supplier_groups = \App\SupplierGroup::forDropdown($business_id);
        return view('contact.create')
            ->with(compact('types', 'store_action', 'supplier_groups','sources', 'life_stages', 'users', 'module_form_parts'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('contact.edit')
            ->with(compact('contact', 'types','supplier_groups' ,'update_action', 'sources', 'life_stages', 'users'));
    }
    ...
    foreach($leads as $row){
        ...
        'company' => $row->supplier_business_name,
        'address' => $row->address_line_1,
        'city' => $row->city,
        'state' => $row->state,
        'default_language' => 'english' ,
            );
            $str=http_build_query($leads_array);
            $url="https://care.msaraterp.com/api/leads";
            $token="eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VyIjoiZmF3YXRyYSIsIm5hbWUiOiJmYXdhdHJhIiwicGFzc3dvcmQiOm51bGwsIkFQSV9USU1FIjoxNjQ2MTY2MzQ0fQ.wRG367Vsx0uECE7y-dGBsJrTsRkaibin5UtVgsWpmEc";
            // Create a new cURL resource
            $ch = curl_init($url);
        // $payload = json_encode(array("user" => $data));
            // Attach encoded JSON string to the POST fields
            curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($leads_array));
            // Set the content type to application/json
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            'authtoken:eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VyIjoiZmF3YXRyYSIsIm5hbWUiOiJmYXdhdHJhIiwicGFzc3dvcmQiOm51bGwsIkFQSV9USU1FIjoxNjQ2MTY2MzQ0fQ.wRG367Vsx0uECE7y-dGBsJrTsRkaibin5UtVgsWpmEc'
            ));
