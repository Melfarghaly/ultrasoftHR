    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        Menu::create('contact-sidebar-menu', function ($menu) {
            ...
            if (in_array($contact->type, ['customer', 'both'])) {
                ...
                $menu->url(action('\Modules\Crm\Http\Controllers\SellController@returnList'), __('lang_v1.sell_return'), ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'contact' && request()->segment(2) == 'contact-sell-return'])->order(3);
                $menu->url(action('\Modules\Crm\Http\Controllers\SellController@create'), __('انشاء عرض سعر'), ['icon' => 'fa fas fa-plus', 'active' => request()->segment(1) == 'contact' && request()->segment(2) == 'sells'])->order(3);
