...
<section class="content">
	@component('components.filters', ['title' => __('report.filters'), 'class' => 'box-solid'])
		...
		<div class="col-md-3">
			<div class="form-group">
				{!! Form::label('manager_id', __('مدير الاجتماع') . ':') !!}
				{!! Form::select('manager_id', $users, null, ['class' => 'form-control select2', 'placeholder' => __('messages.all')]); !!}
			</div>
		</div>
		<div class="col-md-3">
			<div class="form-group">
			{!! Form::label('status', __('نوع الاجتماع ') . ':') !!}
					<select name="meeting_status_id" id="meeting_status_id" class="form-control">
			...
			<table class="table table-bordered table-striped" id="task_table">
				<thead>
					<tr>
						...
						<th class="col-md-2">رئيس الاجتماع</th>
...
<script type="text/javascript">
	$(document).ready(function(){
		task_table = $('#task_table').DataTable({
	        ...
	        ajax: {
	        	...
	        	data: function(d) {
	        		...
	        		d.meeting_status_id = $('#meeting_status_id').val();
	        		var start = '';
	                var end = '';
	        ...
	        columns: [
	            ...
	            { data: 'meeting_status_id', name: 'meeting_status_id' },
		...
		$(document).on('change', "#manager_id, #user_id_filter, #meeting_status_id", function(){
			task_table.ajax.reload();
		});
	});
