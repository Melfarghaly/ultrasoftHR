                                            ...
                                            <tbody>
                                                ...
                                                @if(!empty($payroll['allowances']))
                                                    ...
                                                    @foreach($payroll['allowances']['allowance_names'] as $key => $value)
                                                        @include('essentials::payroll.allowance_and_deduction_row', ['add_button' => $loop->index == 0 ? true : false,'key'=>$key, 'type' => 'allowance', 'name' => $value, 'value' => $payroll['allowances']['allowance_amounts'][$key], 'amount_type' => $payroll['allowances']['allowance_types'][$key],
                                    ...
                                    @component('components.widget')
                                        <table class="table table-condenced deductions_table" id="deductions_table_{{$employee}}" data-id="{{$employee}}">
                                            ...
                                            <tbody>
                                                ...
                                                @else
                                                @php
                                                $fixed_deductions=json_decode(DB::table('users')->where('id',$employee)->first()->fixed_deductions);
                                              // dd($fixed_deductions);
                                                @endphp
                                                    @if(!empty($fixed_deductions))
                                                        ...
                                                        @include('essentials::payroll.edit_allowance_and_deduction_row', 
                                                            ['add_button' => $loop->index == 0 ? true : false, 'type' => 'deduction',  'tax'=>true,
