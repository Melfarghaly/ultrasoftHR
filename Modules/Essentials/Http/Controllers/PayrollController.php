...

{
    ...
    public function index()
    {
            ...
            return Datatables::of($payrolls)
                ->addColumn(
                    ...
                    function ($row) use ($is_admin) {
                        ...
                        if (auth()->user()->can('essentials.edit_payroll')) {
        ...
        $contact_dropdown =\App\Contact::contactDropdown($business_id, false, false);
        return view('essentials::payroll.index')->with(compact('employees','contact_dropdown', 'is_admin','users','departments', 'designations', 'locations'));
    }
...
/*
                    $payroll['allowance_names'][$index]=$request->allowance_names[$index];
                    $payroll['allowance_types'][$index]=$request->allowance_types[$index];
                    $payroll['allowance_percent'][$index]=$request->allowance_percent[$index];
                    $payroll['allowance_amounts'][$index]=$request->allowance_amounts[$index];
                    
                    
                    $payroll['deduction_names'][$index]=$request->deduction_names[$index];
                    $payroll['deduction_types'][$index]=$request->deduction_types[$index];
                    $payroll['deduction_percent'][$index]=$request->deduction_percent[$index];
                    $payroll['deduction_amounts'][$index]=$request->deduction_amounts[$index];
...
//dd($payroll);
    ...
    public function edit($id)
    {
        ...
        if (!(auth()->user()->can('superadmin') || !auth()->user()->can('essentials.edit_payroll') ||!$this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module')) && !auth()->user()->can('essentials.update_payroll')) {
           // abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($payroll_groups)
                ->addColumn(
                    ...
                    function ($row) {
                        ...
                        if ($row->status == 'final' && $row->payment_status != 'paid') {
                            if(auth()->user()->can('essentials.add_payment')){
                            $html .= '<li>
                ...
                ->addColumn('taxes',function($row){
                    $name=__('lang_v1.tax');
                    return $this->al_de_paygroup($row->id,$name);
                    
                })
                ->addColumn('insurances',function($row){
 ...
 public function al_de_paygroup($id,$name){
     $transactions=\DB::table('essentials_payroll_group_transactions')
                 ->join('transactions','essentials_payroll_group_transactions.transaction_id','transactions.id')
        ...
        foreach($transactions as $tr){
                ...
                $deductions=json_decode($tr->essentials_deductions);
                    if(!empty($deductions->deduction_amounts))
                    foreach($deductions->deduction_amounts as $key=>$value){
                        if($deductions->deduction_names[$key]==$name )
                            $output+=$value;
                    }
    ...
    public function viewPayrollGroup($id)
    {
        ...
        return view('essentials::payroll.view_payroll_group_detailed')
    ...
    {
        $business_id = request()->session()->get('user.business_id');
        if (!(auth()->user()->can('superadmin') || auth()->user()->can('essentials.edit_payroll') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module'))) {
            abort(403, 'Unauthorized action.');
        }
...
//dd($pay_components);
    ...
    {
        $query = User::where('business_id', $business_id)->where('status','active');
