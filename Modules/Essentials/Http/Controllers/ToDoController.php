...
class ToDoController extends Controller
{
     ...
     * @return Response
     */
     public function delet_meeting_status($id){
         \DB::table('meeting_status')->where('id',$id)->delete();
           $output = [
                          'success' => true,
     ...
     public function store_status(Request $request ){
         ...
         try{
             \DB::table('meeting_status')->insert(['name'=>$request->name,'business_id'=>$business_id]);
               $output=[
                 'success'=>1,
                 'msg'=>"تمت الاضافة "
                 ];
    ...
    {
        if (!auth()->user()->can('essentials.show')) {
        ...
        if (request()->ajax()) {
            ...
            if (!empty($request->manager_id)) {
                $todos->where('manager_id', $request->manager_id);
            }

            if (!empty($request->meeting_status_id)) {
                $todos->where('meeting_status_id', $request->meeting_status_id);
            }
            ...
            return Datatables::of($todos)
                ...
                ->editColumn('meeting_status_id', function ($row) {
                    $statuses=\DB::table('meeting_status')->where('id',$row->meeting_status_id)->first();
                    if($statuses)
                ...
                ->editColumn('manager_id', function ($row) {
                    $user=\DB::table('users')->where('id',$row->manager_id)->first();
                    if($user)
                ...
                ->removeColumn('id')
                ->rawColumns(['task', 'action', 'status','meeting_status_id','manager_id'])
                ->make(true);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                $users_tansik = $request->input('users_tansik');
                ...
                $to_dos->users_tansik()->sync($users_tansik);
