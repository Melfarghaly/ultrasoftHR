...
class EssentialsAllowanceAndDeductionController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            $q = EssentialsAllowanceAndDeduction::where('business_id', $business_id)
                ->with('employees');
                if(!empty(request()->get('start_date'))  ){
                    $q->where('applicable_date','>=', request()->get('start_date'))
                    ->where('applicable_date','<=', request()->get('end_date'));
                }
                $allowances=$q->get();
