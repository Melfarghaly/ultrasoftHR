...
class AttendanceController extends Controller
{
     ...
     * @return Response
     */
     public function convertExcelDate($dateIntger){
          $excel_date = $dateIntger; //here is that value 41621 or 41631
        ...
        try {
            ...
            if ($request->hasFile('attendance')) {
                ...
                foreach ($imported_data as $key => $value) {
                    //$originalDate = $imported_data[$key][2];
                    if(is_numeric($imported_data[$key][2])){
                        $value[2]=$this->convertExcelDate($imported_data[$key][2]);
                    }
                    if(is_numeric($imported_data[$key][1])){
                        $value[1]=$this->convertExcelDate($imported_data[$key][1]);
                    }
                   // $newDate = date("Y-m-d H:i:s", strtotime($originalDate));
                     //$imported_data[$key][2]=$newDate;
                   // dd($imported_data[$key]);
                    $row_no = $key + 1;
                    $temp = [];
