                ...
                ->addColumn('action', function($row) {
                    ...
                    if (auth()->user()->can("job_sheet.view_assigned") || auth()->user()->can("job_sheet.view_all") || auth()->user()->can("job_sheet.create")) {
                            $html .= '<li>
                                ...
                                <li>
                                <a target="_blank" href="' . action('\Modules\Repair\Http\Controllers\JobSheetController@print_slim', ['id' => $row->id]) . '" class="cursor-pointer">
...
*/
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $input['job_sheet_no'] = $this->commonUtil->generateReferenceNumber('job_sheet', $ref_count, null, $repair_settings['job_sheet_prefix'] ?? null);
            ...
            return redirect()->back()
                ->with('status', ['success' => false,
                    'msg' => __('messages.something_went_wrong').$e->getMessage().'  '.$e->getFile().' '.$e->getLine()
                ]);
        }
    }
