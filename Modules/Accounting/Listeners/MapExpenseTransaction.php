...
class MapExpenseTransaction
{
        ...
        public function handle(ExpenseCreatedOrModified $event)
    {
        ...
        $deposit_to = isset($accounting_default_map['expenses']['deposit_to']) ? $accounting_default_map['expenses']['deposit_to'] : null;
        $payment_account = isset($accounting_default_map['expenses']['payment_account']) ? $accounting_default_map['expenses']['payment_account'] : null;
        
        $tax_account = isset($accounting_default_map['expenses']['tax_account']) ? $accounting_default_map['expenses']['tax_account'] : null;
        ...
        if(isset($event->isDeleted) && $event->isDeleted){
            ...
            if(!is_null($deposit_to) && !is_null($payment_account)){
                ...
                $accountingUtil = new \Modules\Accounting\Utils\AccountingUtil();
                $accountingUtil->saveMap($type, $id, $user_id, $business_id, $deposit_to, $payment_account,$tax_account);
            }
        }
    }
