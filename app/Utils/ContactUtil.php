...
class ContactUtil extends Util
{
    ...
    public function updateOpeningBalance($input, $id)
    {
        ...
        if ($count == 0) {
            ...
            if (! empty($ob_transaction)) {
                ...
                //Add opening balance
                if (! empty($opening_balance)) {
                    $transactionUtil->createOpeningBalance( $contact->id, $opening_balance, $contact->created_by, false);
                }
            }
    ...
    public function getOpeningBalance($type, $contact_ids = [])
    {
        $query = Contact::leftjoin('transactions AS t', 'contacts.id', '=', 't.contact_id')
                    ->leftjoin('customer_groups AS cg', 'contacts.customer_group_id', '=', 'cg.id');
  ...
  public function getAllOpeningBalance($type)
    {
        $query = Contact::leftjoin('transactions AS t', 'contacts.id', '=', 't.contact_id')
                    ->leftjoin('customer_groups AS cg', 'contacts.customer_group_id', '=', 'cg.id');
   ...
   public function updateAllOpeningBalance($input, $id)
    {
