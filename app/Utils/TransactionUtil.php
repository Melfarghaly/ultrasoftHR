...
class TransactionUtil extends Util
{
    ...
    public function createOpeningBalance( $contact_id, $amount, $created_by, $uf_data = true)
    {
        $contact = Contact::findOrFail($contact_id);
        $business_location = BusinessLocation::where('business_id', $contact->business_id)
                                                        ->first();
        $final_amount = $uf_data ? $this->num_uf($amount) : $amount;
        $ob_data = [
        ...
        //Update reference count
        $ob_ref_count = $this->setAndGetReferenceCount('opening_balance', $contact->business_id);
        //Generate reference number
        $ob_data['ref_no'] = $this->generateReferenceNumber('opening_balance', $ob_ref_count, $contact->business_id);
        //Create opening balance transaction
        Transaction::create($ob_data);
    }
